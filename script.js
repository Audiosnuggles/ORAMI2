/**
 * THE PIGEON - Final Master v13 (Brush Persistence Edition)
 * - Speichert und lädt Pinsel-Stil und Stärke pro Segment
 * - Verhindert "Shared Memory" durch Deep Copy
 * - Lädt "default_set.json"
 */
...
function drawSegmentCalligraphy(ctx, pts, idx1, idx2, size) { const angle = -Math.PI / 4, dx = Math.cos(angle) * (size/2), dy = Math.sin(angle) * (size/2); ctx.fillStyle = "#000"; ctx.beginPath(); ctx.moveTo(pts[idx1].x - dx, pts[idx1].y - dy); ctx.lineTo(pts[idx1].x + dx, pts[idx1].y + dy); ctx.lineTo(pts[idx2].x + dx, pts[idx2].y + dy); ctx.lineTo(pts[idx2].x - dx, pts[idx2].y - dy); ctx.fill(); }
function drawSegmentParticles(ctx, pts, idx1, idx2, size) { ctx.fillStyle = "rgba(0,0,0,0.6)"; for(let i=0; i<2; i++) { const ox = (Math.random()-0.5)*size*2, oy = (Math.random()-0.5)*size*2; ctx.beginPath(); ctx.arc(pts[idx2].x+ox, pts[idx2].y+oy, Math.max(1, size/4), 0, Math.PI*2); ctx.fill(); } }
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // === INIT PATTERN MACHINE ===
  const savedBanks = localStorage.getItem("pigeonBanks");
  if (savedBanks) {
      try { patternBanks = JSON.parse(savedBanks); updatePadUI(); } catch(e) { localStorage.removeItem("pigeonBanks"); loadDefaultSet(); }
  } else { loadDefaultSet(); }

  function loadDefaultSet() {
      fetch('default_set.json').then(res => res.json()).then(data => {
          if(data.banks) { patternBanks = data.banks; updatePadUI(); }
          if(data.current) loadPatternData(data.current);
      }).catch(err => console.log("Kein default_set.json gefunden."));
  }
  ...
  saveModeBtn.addEventListener("click", () => { isSaveMode = !isSaveMode; saveModeBtn.classList.toggle("active", isSaveMode); });
  ...
  pads.forEach(pad => {
      pad.addEventListener("click", () => {
          ...
          if (isSaveMode) {
              ...
              patternBanks[b][i] = JSON.parse(JSON.stringify(stateToSave));
              ...
              if (data) {
                  if (isPlaying) { queuedPattern = { data: data, pad: pad }; pads.forEach(p => p.classList.remove("queued")); pad.classList.add("queued");
                  } else { loadPatternData(data); pads.forEach(p => { p.classList.remove("active"); p.classList.remove("queued"); }); pad.classList.add("active"); }
              }
          }
      });
  });
  ...
  function loadPatternData(d) {
      ...
      if(Array.isArray(trackData)) {
          trackData.forEach((td, idx) => {
              ...
              let t = tracks[idx];
              if (Array.isArray(td)) { t.segments = JSON.parse(JSON.stringify(td)); } else {
                  t.segments = JSON.parse(JSON.stringify(td.segments || []));
                  t.vol = td.vol !== undefined ? td.vol : t.vol;
                  t.mute = td.mute !== undefined ? td.mute : t.mute;
                  t.wave = td.wave || t.wave;
                  t.snap = td.snap !== undefined ? td.snap : t.snap;
      ...
      if (isPlaying) { const bpmVal = parseFloat(document.getElementById("bpmInput").value); playbackDuration = (60 / (bpmVal || 120)) * 32; }
  }
  ...
  function startLiveSynth(track, y) {
    ...
    intervals.forEach(iv => {
      ...
      if (Number.isFinite(fVal) && fVal > 0) { osc.frequency.setValueAtTime(fVal, audioCtx.currentTime); let out = osc; if(brush==="fractal") { const shaper = audioCtx.createWaveShaper(); shaper.curve = getDistortionCurve(); osc.connect(shaper); out = shaper; } out.connect(liveGainNode); osc.start(); liveNodes.push(osc); }
    });
  }
  ...
  function updateLiveSynth(track, y) {
    ...
    liveNodes.forEach((node, i) => { if(node.frequency) { const ivs = (brushSelect.value==="chord") ? chordIntervals[chordSelect.value] : [0]; const iv = ivs[i] || 0; const fVal = freq * Math.pow(2, iv/12); if (Number.isFinite(fVal) && fVal > 0) node.frequency.setTargetAtTime(fVal, audioCtx.currentTime, 0.02); } });
  }
  ...
  function triggerParticleGrain(track, y) {
    ...
    const tg = audioCtx.createGain(); tg.gain.value = track.vol; osc.connect(env).connect(tg).connect(masterGain); osc.start(); osc.stop(audioCtx.currentTime+0.2); setTimeout(()=>tg.disconnect(), 250);
  }

  function scheduleTracks(start, targetCtx = audioCtx, targetDest = masterGain) {
    tracks.forEach(track => {
      const trkGain = targetCtx.createGain(); trkGain.connect(targetDest); trkGain.gain.value = track.mute ? 0 : track.vol;
      if (targetCtx === audioCtx) track.gainNode = trkGain;
      track.segments.forEach(seg => {
        const brush = seg.brush || "standard"; const thickness = seg.thickness || 5;
        if(brush==="particles") {
           seg.points.forEach(p => {
             const t = Math.max(0, start + (p.x/track.canvas.width)*playbackDuration); if(!Number.isFinite(t)) return;
             const osc = targetCtx.createOscillator(); osc.type = track.wave; let f = mapY(p.y, track.canvas.height); if(harmonizeCheckbox.checked) f = quantize(f);
        ...
        const osc = targetCtx.createOscillator(); osc.type = track.wave; const g = targetCtx.createGain(); g.gain.setValueAtTime(0, sT); g.gain.linearRampToValueAtTime(0.3, sT+0.02); g.gain.setValueAtTime(0.3, eT); g.gain.linearRampToValueAtTime(0, eT+0.1);
        if(brush==="fractal"){ const shaper=targetCtx.createWaveShaper(); shaper.curve=getDistortionCurve(); osc.connect(shaper); shaper.connect(g); } else { osc.connect(g); }
        g.connect(trkGain); sorted.forEach(p => { const t = Math.max(0, start + (p.x/track.canvas.width)*playbackDuration); let f = mapY(p.y, track.canvas.height); if(harmonizeCheckbox.checked) f = quantize(f); if(Number.isFinite(f) && Number.isFinite(t) && f>0) osc.frequency.linearRampToValueAtTime(f, t); });
  ...
  function updatePigeonViz(currentX) {
     ...
     if (currentX !== undefined) tracks.forEach(t => { if(t.mute) return; t.segments.forEach(seg => { if (seg.brush === "fractal") { const xs = seg.points.map(p => p.x); if (currentX >= Math.min(...xs) && currentX <= Math.max(...xs)) isFractalActive = true; } }); });
     let transformStr = "scale(1)", filterStr = "";
     if (isFractalActive) { if(avg > 10) { const rOff = (avg/10) * (Math.random()>0.5?1:-1); filterStr = `drop-shadow(${rOff}px 0px 0px rgba(255,0,0,0.7))`; }
     } else { const kickForce = Math.max(0, diff); const squashY = 1 - Math.min(0.5, (kickForce / 100) * 1.9); const stretchX = 1 + Math.min(0.2, (kickForce / 100) * 0.5); transformStr = `scale(${stretchX}, ${squashY})`; }
  ...
  function loop() {
    ...
    if(elapsed >= playbackDuration) {
       if (queuedPattern) { loadPatternData(queuedPattern.data); pads.forEach(p => { p.classList.remove("active"); p.classList.remove("queued"); }); queuedPattern.pad.classList.add("active"); queuedPattern = null; }
       if(document.getElementById("loopCheckbox").checked) { playbackStartTime = audioCtx.currentTime; scheduleTracks(playbackStartTime); } else { document.getElementById("stopButton").click(); return; }
    }
    const x = (elapsed/playbackDuration) * 750; tracks.forEach(t => redrawTrack(t, x)); updatePigeonViz(x);
  ...
  function quantize(f) { const s=document.getElementById("scaleSelect").value; let m=69+12*Math.log2(f/440), r=Math.round(m), pat=(s==="major")?[0,2,4,5,7,9,11]:(s==="minor")?[0,2,3,5,7,8,10]:[0,3,5,7,10], mod=r%12, b=pat[0], md=99; pat.forEach(p=>{let d=Math.abs(p-mod); if(d<md){md=d;b=p;}}); return 440*Math.pow(2,(r-mod+b-69)/12); }
  ...
  // --- DIE INTELLIGENTE REDRAW-FUNKTION (Nutzt jetzt seg.brush & seg.thickness) ---
      ...
      t.segments.forEach(seg => {
          ...
          if (brush === "chord") {
              chordIntervals[seg.chordType || "major"].forEach((iv, i) => {
                  t.ctx.strokeStyle = chordColors[i % 3]; t.ctx.lineWidth = thickness; t.ctx.beginPath(); t.ctx.moveTo(pts[0].x, pts[0].y - iv * 5);
                  for (let k = 1; k < pts.length; k++) t.ctx.lineTo(pts[k].x, pts[k].y - iv * 5); t.ctx.stroke();
              });
          } else if (brush === "particles") {
              for (let i = 1; i < pts.length; i++) drawSegmentParticles(t.ctx, pts, i - 1, i, thickness);
          } else {
              for (let i = 1; i < pts.length; i++) {
                  switch (brush) {
                      case "variable": drawSegmentVariable(t.ctx, pts, i - 1, i, thickness); break;
                      case "calligraphy": drawSegmentCalligraphy(t.ctx, pts, i - 1, i, thickness); break;
                      case "fractal": drawSegmentFractal(t.ctx, pts, i - 1, i, thickness); break;
                      default: drawSegmentStandard(t.ctx, pts, i - 1, i, thickness);
                  }
              }
          }
      });
  ...
  // --- UI BUTTONS ---
  document.getElementById("playButton").addEventListener("click", () => { if(isPlaying) return; initAudio(); if(audioCtx.state==="suspended") audioCtx.resume(); const bpmVal = parseFloat(document.getElementById("bpmInput").value); playbackDuration = (60/(bpmVal||120))*32; playbackStartTime = audioCtx.currentTime+0.1; isPlaying=true; scheduleTracks(playbackStartTime); requestAnimationFrame(loop); });
  document.getElementById("stopButton").addEventListener("click", () => { isPlaying=false; queuedPattern=null; cancelAnimationFrame(animationFrameId); tracks.forEach(t=>{if(t.gainNode){t.gainNode.disconnect();t.gainNode=null;} redrawTrack(t);}); if(pigeonImg) { pigeonImg.style.transform="scale(1)"; pigeonImg.style.filter=""; } pads.forEach(p => p.classList.remove("queued")); });
  document.getElementById("clearButton").addEventListener("click", () => { tracks.forEach(t=>{t.segments=[]; redrawTrack(t);}); undoStack=[]; pads.forEach(p => { p.classList.remove("active"); p.classList.remove("queued"); }); queuedPattern = null; });
  ...
  document.getElementById("exportButton").addEventListener("click", () => { const data = { current: { settings: { bpm: document.getElementById("bpmInput").value, loop: document.getElementById("loopCheckbox").checked, scale: document.getElementById("scaleSelect").value, harmonize: document.getElementById("harmonizeCheckbox").checked }, tracks: tracks.map(t => ({ segments: t.segments, vol: t.vol, mute: t.mute, wave: t.wave, snap: t.snap })) }, banks: patternBanks }; const blob = new Blob([JSON.stringify(data)], {type:"application/json"}); const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="pigeon_live_set.json"; a.click(); });
  document.getElementById("importButton").addEventListener("click", () => document.getElementById("importFileInput").click());
  document.getElementById("importFileInput").addEventListener("change", e => { const r = new FileReader(); r.onload = evt => { try { const d=JSON.parse(evt.target.result); if(d.banks) { patternBanks = d.banks; localStorage.setItem("pigeonBanks", JSON.stringify(patternBanks)); updatePadUI(); } loadPatternData(d.current || d); } catch(e){ alert("Fehler!"); } }; r.readAsText(e.target.files[0]); e.target.value = ''; });
  ...
  function audioBufferToWav(buffer) {
      ...
      function setUint16(data) { view.setUint16(offset, data, true); offset += 2; } function setUint32(data) { view.setUint32(offset, data, true); offset += 4; }
  ...
  document.getElementById("exportWavButton").addEventListener("click", () => {
      ...
      setTimeout(() => { 
          ...
          const tempDur = playbackDuration; playbackDuration = targetDuration;
          const offlineCtx = new (window.OfflineAudioContext || window.webkitOfflineAudioContext)(2, 44100 * targetDuration, 44100);
          const master = offlineCtx.createGain(); master.gain.value = 0.5; master.connect(offlineCtx.destination);
          scheduleTracks(0, offlineCtx, master);
          offlineCtx.startRendering().then(renderedBuffer => {
              const a = document.createElement("a"); a.href = URL.createObjectURL(audioBufferToWav(renderedBuffer)); a.download = `pigeon_${targetBpm}bpm.wav`; a.click();
              btn.innerText = "Export WAV"; playbackDuration = tempDur;
          });
      }, 50);
  });
});